...
export async function GET() {
  try {
    const result = await swr<OverviewData>(
      CACHE_KEY,
      { freshTtlS: freshTtlS() },
      async () => {
        ...
        // Build nav-by-symbol map from the Centrifuge indexer's tokenPrice
        // field. Source of truth for NAV; only supply comes from on-chain.
