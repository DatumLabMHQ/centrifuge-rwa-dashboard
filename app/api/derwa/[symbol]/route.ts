...
export async function GET(
  ...
  try {
    ...
    // Tier 1 on-chain supply (authoritative TVL source).
    ...
    if (morphoIntegration?.address) {
      ...
      if (stats) {
        morpho = {
          ...
          collateralAddress: stats.collateralAddress,
          loanSymbol: stats.loanSymbol,
          loanAddress: stats.loanAddress,
          ...
          oracleType: stats.oracleType,
          oracleAddress: stats.oracleAddress,
          dailyPriceVariation: stats.dailyPriceVariation,
          badDebtUsd: stats.badDebtUsd,
          collateralRatio: stats.collateralRatio,
          distanceToLiquidation: stats.distanceToLiquidation,
          historicalSupplyUsd: stats.historicalSupplyUsd,
          historicalBorrowUsd: stats.historicalBorrowUsd,
          historicalUtilization: stats.historicalUtilization,
          historicalSupplyApy: stats.historicalSupplyApy,
          historicalBorrowApy: stats.historicalBorrowApy,
          irmCurve: stats.irmCurve,
        };
      }
    }

    // Tier 1 vs Tier 2 reconciliation for this wrapper only.
    const allRecons = reconcileAll({ pools, onchainSupplies });
